...
import { Receipt, Search, Lock, Unlock, ShieldAlert, CheckCircle, Eye, Trash2, XCircle } from 'lucide-react';
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-800 pb-6">
        <div className="space-y-1">
          <h1 className="text-3xl font-black text-white tracking-tight flex items-center space-x-3">
            ...
            <span>Cryptographic Spend Ledger</span>
          </h1>
          <p className="text-slate-400 text-sm">
            Historical audit of autonomous Machine-to-Machine (M2M) transactions, cryptographic signatures, and network overlay tunnels.
          </p>
        </div>
        <button
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-3xl overflow-hidden">
        {filteredLedger.length === 0 ? (
          <div className="p-16 text-center text-slate-500 italic">
            ...
            <p className="text-xs text-slate-600 mt-1 font-mono">Modify search query or filter criteria.</p>
          </div>
        ) : (
            ...
            <table className="w-full text-left text-xs font-mono">
              <thead>
                <tr className="bg-slate-950 text-slate-400 border-b border-slate-800 font-bold uppercase tracking-wider text-[10px]">
                  ...
                  <th className="py-4 px-6">Order Intent</th>
                  <th className="py-4 px-6">Merchant</th>
                  <th className="py-4 px-6">Amount</th>
                  <th className="py-4 px-6">Guard Filter</th>
                    ...
                    <td className="py-4 px-6">
                      <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-bold border flex items-center space-x-1 w-fit ${
                        ...
                        {item.status === 'SUCCESS' ? (
                          ...
                          <>
                            <ShieldAlert size={10} />
                            <span>{item.status === 'BLOCKED' ? 'BLOCKED' : 'FAILED'}</span>
                          </>
                        )}
                      </span>
                    </td>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 max-w-2xl w-full max-h-[90vh] overflow-y-auto flex flex-col justify-between">
            <div>
              <div className="flex items-center justify-between pb-4 border-b border-slate-800 mb-6">
                <div>
                  <h3 className="text-lg font-bold text-white flex items-center space-x-2">
                    ...
                    <span>Machine Transaction Audit</span>
                  </h3>
                  <p className="text-xs text-slate-500 font-mono mt-0.5">
                    Transaction ID: {inspectItem.transactionId || inspectItem.id}
                  </p>
                </div>
                <button
              ...
              <div className="space-y-4 font-mono text-xs mb-6">
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-slate-950 p-4 rounded-xl border border-slate-850 space-y-1">
                    <span className="text-slate-500 text-[10px]">TRANSACTION TIMESTAMP</span>
                    <span className="text-slate-200 font-bold block">{inspectItem.timestamp}</span>
                  </div>
                  <div className="bg-slate-950 p-4 rounded-xl border border-slate-850 space-y-1">
                    <span className="text-slate-500 text-[10px]">REGISTERED MERCHANT</span>
                ...
                <div className="bg-slate-950 p-4 rounded-xl border border-slate-850 space-y-1.5">
                  <span className="text-slate-500 text-[10px] block">WALLET COMPLIANCE</span>
                  <div className="text-[10px] text-slate-400 space-y-1 leading-relaxed">
                    <p>Final Status: {inspectItem.status}</p>
                    <p>Guard Filter: {inspectItem.securityCheck === 'PASSED' ? '✅ COMPLIANT' : '❌ POLICY VIOLATION'}</p>
                    {inspectItem.securityReason && (
                      <p className="text-rose-400 mt-1">Block Reason: {inspectItem.securityReason}</p>
                    )}
                    {inspectItem.authCode && <p className="text-emerald-400">Bank Auth Code: {inspectItem.authCode}</p>}
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-2 font-mono text-xs">
                <span className="text-slate-400 font-bold block">Signed x402 Cryptographic Payload:</span>
              ...
              {/* Ziti Routing & AI Reasoning Logs */}
              <div className="space-y-2 mt-4 font-mono text-xs">
                <span className="text-slate-400 font-bold block">OpenZiti Connectivity Logs & AI Reasoning:</span>
